...
auto c1 = coek::constraint("c1", x + y == 1);
model.add(c1);

auto c2 = coek::constraint("c2", (2 * x) + y <= 1);
model.add(c2);

auto c3 = coek::constraint("c3", y - (2 * x) >= 1);
model.add(c3);
...
auto c = coek::constraint("c", (x(1) * (1 + coek::pow(x(2), 2))) + coek::pow(x(3), 4) == 4 + (3 * coek::sqrt(2)));
model.add(c);
