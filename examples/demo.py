...
c1 = smk.constraint(expr = 3 * v[i] - smk.sin(x[j] + 4) >= 0, name='c1').forall(i, In=S).forall(j, In=J)

c2 = smk.constraint(expr = M[i, j] / x[j] <= v[i] * z[i], name='c2').forall(i, In=I).forall(j, In=J)

c3 = smk.constraint(expr = smk.sum(v[i]**2 + x[j]**2).forall(i, In=I).forall(j, In=J) <= 100, name='c3')
...
# ex1 = smk.expression(name='ex1', expr=v[i]**3 + smk.sin(x[j] + 4))
