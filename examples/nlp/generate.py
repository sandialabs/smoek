...
with open("models/smoek_generate.cpp", "w") as OUTPUT:
    generate_fn = "\n".join(
        f'if (testname == "{name}") return generate_{name}(data, func_ptr, last_x, last_g);'
        for name in testnames
    )
    generate_decl = "\n".join(
        f"IpoptProblem generate_{name}(const coek::DataPortal& data, CreateIpoptProblem_func_t func_ptr, std::vector<Number>& last_x, std::vector<Number>& last_g);"
        for name in testnames
    )
